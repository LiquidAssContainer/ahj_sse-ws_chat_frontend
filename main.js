...
var es_array_concat = __webpack_require__(61);
...
var es_string_iterator = __webpack_require__(78);
...
// CONCATENATED MODULE: ./src/js/UsernameForm.js
...
var UsernameForm = /*#__PURE__*/function () {
  function UsernameForm(chat) {
  ...
  _createClass(UsernameForm, [{
    ...
    value: function registerEvents() {
      ...
      var form = document.getElementsByClassName('modal_form')[0];
      var input = document.getElementsByClassName('form_input')[0];
      form.addEventListener('submit', function (e) {
        e.preventDefault();

        _this.chat.createUser(input.value);
      });
    }
  }]);
...
var API = /*#__PURE__*/function () {
  ...
  API_createClass(API, [{
    ...
    key: "registerWsEvents",
    value: function registerWsEvents(ws) {
      var _this2 = this;
      ...
      ws.addEventListener('message', function (e) {
        try {
          ...
          switch (type) {
            case 'userId':
              _this2.chat.openChat();

              _this2.chat.userId = data.id;
              _this2.chat.username = data.username;
              break;

            case 'message':
              _this2.chat.addNewMessage(data);

              break;

            case 'userCame':
              _this2.chat.addUser(data);

              break;

            case 'userGone':
              _this2.chat.removeUser(data);

              break;

            case 'usernameError':
              _this2.chat.showError(data);
...
// CONCATENATED MODULE: ./src/js/createErrorPopup.js
...
var Chat_Chat = /*#__PURE__*/function () {
  function Chat(url) {
    ...
    var _document$getElements = document.getElementsByClassName('chat_users');

    var _document$getElements2 = _slicedToArray(_document$getElements, 1);

    this.users = _document$getElements2[0];

    var _document$getElements3 = document.getElementsByClassName('chat_messages');

    var _document$getElements4 = _slicedToArray(_document$getElements3, 1);

    this.chatMessages = _document$getElements4[0];
  ...
  Chat_createClass(Chat, [{
    ...
    value: function addUser(username) {
      var isYou = this.username === username;
      var userElem = "\n      <div class=\"chat_user\" data-username=\"".concat(username, "\">\n        <div class=\"chat_user_avatar\"></div>\n        <div class=\"chat_user_name").concat(isYou ? ' chat_user_you' : '', "\">\n          ").concat(username).concat(isYou ? ' (you)' : '', "\n        </div>\n      </div>");
      this.users.insertAdjacentHTML('beforeend', userElem);
    }
  }, {
    key: "removeUser",
    value: function removeUser(username) {
      var userElem = document.querySelector("[data-username=\"".concat(username, "\"]"));
      userElem === null || userElem === void 0 ? void 0 : userElem.remove();
    }
  }, {
    key: "showError",
    ...
    value: function addNewMessage(data) {
      ...
      var isScrollAtBottom = this.isScrollAtBottom(chatMessages);
      var messageElem = this.getMessageHTML(data);
    ...
    key: "getMessageHTML",
    value: function getMessageHTML(data) {
      var date = new Date(data.date);
      var formattedDate = this.getFormattedDate(date);
      var isYou = this.username === data.username;
      var username = isYou ? 'You' : data.username;
      return "\n      <div class=\"chat_message".concat(isYou ? ' chat_message_yours' : '', "\">\n        <div class=\"chat_message_username\">").concat(username, ", ").concat(formattedDate, "</div>\n        <div class=\"chat_message_content\">").concat(data.message, "</div>\n      </div>");
    }
  }, {
    ...
    value: function registerEvents() {
      ...
      document.addEventListener('keydown', function (e) {
        if (e.key === 'Enter' && !e.shiftKey && document.activeElement === _this.textarea) {
          e.preventDefault();
          var value = e.target.value;
          e.target.value = '';

          _this.API.sendMessage('message', value);
        }
      });
    }
  }]);
